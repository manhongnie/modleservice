...
- **网页测试**：浏览器选择已启用模型，填写文本或上传图片、音频，直接查看和下载推理结果。
...
启动服务后，在浏览器访问 **http://127.0.0.1:8000/**：

1. 输入服务启动时设置的 **业务密钥 `BUSINESS_API_KEY`**，连接并加载可用模型。
2. 选择模型和能力，填写文本或上传图片、WAV 音频；也可以切换到 JSON 输入模式。
3. 点击运行，查看文字、向量、排序分数或图片结果；生成的音频和视频可直接播放、下载。

页面支持对话增量输出、取消接收、原始 JSON 和请求示例。密钥只保留在当前页面内存中，刷新后需要重新输入；“清除密钥”会断开连接。取消请求后，服务仍会等待底层执行真正停止再释放资源。

网页与 API 使用同一端口，无需 Node.js、前端构建或额外服务。模型列表来自 `GET /v1/models`，只返回已验证、已启用且未暂停模型的名称、能力和输入限制，不需要管理密钥。没有可用模型时，先按上文登记 Mock 模型，或按下文准备和登记真实模型。Mock 结果在页面中明确标记。

图片和音频文件在浏览器中转为 Base64 后通过原有推理 API 提交。音频使用单声道 PCM16 WAV；语音识别默认最长 30 秒，克隆参考音频默认 1–15 秒。详细输入限制以对应模型指南及服务校验为准。

其他机器访问时，启动命令增加 `--host 0.0.0.0`，浏览器使用服务器 IP。使用反向代理时，需同时转发 `/`、`/playground.css`、`/playground.js` 和 `/v1/`；已提供对应的 [nginx 模板](deploy/nginx.conf.example)。
...
`GET /v1/models` 使用同一业务密钥获取可用模型目录，响应中的 `model_id` 可以直接用于推理请求。目录不包含模型文件路径、验证样本或后端凭据。
